...
class TradingEngine:
    ...
    def run_algorithms(self, algorithmsInstance):
        ...
        for day in range(self.totalDays -1 ):
            ...
            try:
                ...
                print(f"HANDLED ERROR {e} on DAY {day}.")
                for instrument in self.positionLimits.keys():
    ...
    # save to a text file, the profit history array and other useful information
    def save_key_outputs(self):
        # Get the directory where the script is located
        # Assume parent folder is the team name (and that this is a unique team ID)
        script_directory = os.path.dirname(os.path.abspath(__file__))
        parent_folder = os.path.basename(script_directory)
        
        # Open the text file in write mode
        with open(f'./simulation_results/pnl_{parent_folder}.txt', 'w') as f:
            # Save the current standard output (usually the console)
            original_stdout = sys.stdout
            # Redirect standard output to the file
            sys.stdout = f
            # Print statements will now go to the file:
            # Save array of cumulate PNL
            print(f"{[float(x) for x in self.totalValueHistory]}")
            # Reset the standard output back to the console
            sys.stdout = original_stdout

        # Open the text file in write mode
        with open(f'./simulation_results/results_{parent_folder}.txt', 'w') as f:
            # Save the current standard output (usually the console)
            original_stdout = sys.stdout
            # Redirect standard output to the file
            sys.stdout = f
            # Print statements will now go to the file:
            # daily return vals
            valueHist = np.array(self.totalValueHistory)
            returnsAUD = np.diff(valueHist)
            avgRet = np.mean(returnsAUD)
            stdDev = np.std(returnsAUD)
            sharpe = avgRet / stdDev
            print(f"Average Return: {avgRet:.4f}")
            print(f"Standard Deviation of Returns: {stdDev:.4f}")
